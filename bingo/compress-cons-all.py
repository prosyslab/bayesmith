...
def makeNewRule(ruleProb):
    ...
    while 'R{0}'.format(ruleIndex) in ruleProbs:
        ruleIndex = random.randint(0, 10 * len(ruleProbs))
...
baseQueries = {
    line.strip()
    for line in open(baseQueriesFileName) if len(line.strip()) > 0
}
...
while len(eliminableTuples) > 0:
    ...
    logging.info('Source clause: {0}. Probability: {1}.'.format(
        ', '.join(srcClause), srcProb))
    logging.info('Sink clause: {0}. Probability: {1}.'.format(
        ', '.join(sinkClause), sinkProb))
    logging.info('New clause: {0}. Probability: {1}.'.format(
        ', '.join(newClause), newClauseProb))
    ...
    for tp in set(clause2Antecedents(srcClause)):
        assert not tp == t
        logging.info('Removing srcClause={0} from sinkClauses[tp={1}].'.format(
            ', '.join(srcClause), tp))
    ...
    for tp in set(clause2Antecedents(sinkClause)):
        if tp == t: continue
        logging.info(
            'Removing sinkClause={0} from sinkClauses[tp={1}].'.format(
                ', '.join(sinkClause), tp))
...
logging.info('Maximum new clause length: {0}.'.format(
    max([len(clause) for clause in allClauses])))
