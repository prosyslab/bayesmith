...
oracleQueries = set([
    line.strip() for line in open(oracleQueriesFileName)
    if len(line.strip()) > 0
])
baseQueries = set([
    line.strip() for line in open(baseQueriesFileName) if len(line.strip()) > 0
])
...
with subprocess.Popen([wrapperExecutable, fgFileName], \
    ...
    def observe(t, value):
        assert t not in labelledTuples, 'Attempting to relabel alarm {0}'.format(
            t)
        if not value == (t in oracleQueries):
            logging.warning(
                'Labelling alarm {0} with value {1}, which does not match ground truth.'
                .format(t, value))
    ...
    def getRankedAlarms():
        ...
        def getLabelInt(t):
            return 0 if t not in labelledTuples else 1 if labelledTuples[
                t] else -1

        return sorted(alarmList,
                      key=lambda rec: (-getLabelInt(rec[0]), -rec[1], rec[0]))
    ...
    def printRankedAlarms(outFile):
        ...
        for t, confidence in alarmList:
            ...
            label = 'Unlabelled' if t not in labelledTuples else \
                    ...
                    'NegLabel'
            print('{0}\t{1}\t{2}\t{3}\tSPOkGoodGood\t{4}'.format(
                index, confidence, ground, label, t),
                  file=outFile)

    def runAlarmCarousel(dfile, tolerance, minIters, maxIters, histLength,
                         statsFile, combinedPrefix, combinedSuffix, oneShot):
        assert 0 < tolerance and tolerance < 1
        assert 0 < histLength and histLength < minIters and minIters < maxIters

        numTrue = 0
        numFalse = 0
        with open('{0}{1}.{2}'.format(combinedPrefix, 'init', combinedSuffix),
                  'w') as outFile:
            execWrapperCmd('BP {0} {1} {2} {3}'.format(tolerance, minIters,
                                                       maxIters, histLength))
            printRankedAlarms(outFile)

        if consFileName is not None and printGraph:
            outFile = '{0}{1}.{2}.svg'.format(combinedPrefix, 'init',
                                              combinedSuffix)
            printNetwork(outFile)
            graph.print_node_id(
                network['graph'],
                '{}init.{}.map'.format(combinedPrefix, combinedSuffix))

        if oneShot:  # do not run interation
            return
        ...
        while baseQueries - set(labelledTuples.keys()):
            yetToConvergeFraction = float(
                execWrapperCmd('BP {0} {1} {2} {3}'.format(
                    tolerance, minIters, maxIters, histLength)))
            rankedAlarmList = getRankedAlarms()
            unlabelledAlarms = [(t, confidence)
                                for t, confidence in rankedAlarmList
                                if t not in labelledTuples]
            ...
            outFileName = '{0}{1}.{2}'.format(combinedPrefix,
                                              numTrue + numFalse - 1,
                                              combinedSuffix)
            ...
            logging.info('Setting tuple {0} to value {1}'.format(
                t0, t0 in oracleQueries))
    ...
    for command in sys.stdin:
        ...
        components = [
            c.strip() for c in re.split(' |\t', command) if len(c.strip()) > 0
        ]
        ...
        if cmdType == 'Q':
            ...
            oneShot = True if components[8] == "True" else False
            ...
            with open(statsFileName, 'w') as statsFile:
                runAlarmCarousel(dfile, tolerance, minIters, maxIters, histLength, statsFile, \
                                 combinedPrefix, combinedSuffix, oneShot)
